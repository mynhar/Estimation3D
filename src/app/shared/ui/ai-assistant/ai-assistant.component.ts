...
/**
 * Asistente IA — chat contextual por expediente, consciente del rol.
 *
 * Compartido por las cuatro variantes de rol (cliente, estimador, constructor,
 * administrador). Cada wrapper de rol le pasa su `rol` vía `input()`; si no se
 * pasa, se detecta desde el perfil. El rol determina qué expedientes se
 * consultan, las preguntas frecuentes sugeridas y la presentación (eyebrow,
 * tagline, intro).
 */
@Component({
...
export class AiAssistantComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      // El rol lo fija el wrapper de la ruta; si no, se lee del perfil.
      let rol = this.rolForzado();
      if (!rol) {
  ...
  private async cargarExpedientesPorRol(rol: string, userId: string): Promise<ExpedienteOpcion[]> {
    // El constructor consulta los expedientes en los que participa (los que ha
  ...
  /** Expedientes ofertados por el constructor + disponibles, sin duplicados. */
  private async cargarExpedientesConstructor(constructorId: string): Promise<ExpedienteOpcion[]> {
    const [ofertadas, disponibles] = await Promise.all([
      this.ofertaRepo.findByConstructorIdConExpediente(constructorId),
      this.expRepo.findDisponibles(),
    ]);

    const porId = new Map<string, ExpedienteOpcion>();
    // Primero los propios (ofertados / aceptados); luego los disponibles nuevos.
    for (const o of ofertadas) {
      const exp = o.expediente;
      if (exp?.id) porId.set(exp.id, { id: exp.id, numero: exp.numero, estado: exp.estado ?? '' });
    }
    for (const e of disponibles) {
      if (!porId.has(e.id)) porId.set(e.id, { id: e.id, numero: e.numero, estado: e.estado });
    }
    return [...porId.values()];
  }
