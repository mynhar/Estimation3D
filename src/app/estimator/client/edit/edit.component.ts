...
export class EstimatorClientEditComponent implements OnInit {
  ...
  /**
   * Resend rechaza el remitente propio si el dominio deja de estar verificado y
  ...
  /** El estimador gestiona los dos roles externos y puede mover la cuenta entre ellos. */
  ...
  form = this.fb.group({
    ...
    // Compañía: sólo se envía cuando el rol final es constructor.
    compania_nombre:    [''],
    compania_telefono:  [''],
  ...
  /**
   * Guarda el formulario. Lanza si la edge function falla — quien llama decide
   * qué hacer entonces; lo usan «Guardar cambios» y «Enviar invitación».
   */
  ...
  // implica fijar una nueva. Nunca se genera: la escribe el estimador en
  // «Nueva contraseña», y es obligatoria para invitar. Por eso hay confirmación.

  /** Contraseña escrita por el estimador, si la hay. */
  get passwordEscrita(): string {
    return (this.f['password'].value ?? '').trim();
  }

  abrirInvitacion(): void {
    // El botón también guarda, así que exige un formulario válido completo.
    if (this.form.invalid) { this.form.markAllAsTouched(); return; }
    // Y una contraseña escrita: en esta pantalla es opcional para guardar, pero
    // obligatoria para invitar — es la que va en el correo.
  ...
  /**
   * «Enviar invitación»: guarda los cambios igual que «Guardar cambios» y
   * además envía al usuario sus credenciales (usuario, contraseña y dirección
   * de la aplicación), redactadas en el idioma que tiene registrado.
   *
   * Si el correo falla, los cambios quedan guardados: se avisa con un toast.
   * Se vuelve a la lista salvo que el envío haya caído en el remitente de
   * pruebas, aviso que se queda en pantalla para no perderlo.
   */
  async enviarInvitacion(): Promise<void> {
    ...
    try {
      ...
      this.confirmarInvitacion.set(false);
      this.toast.show(this.translate.instant('admin_users.invite_ok', { email: resultado.email }), 'success');

      if (!this.envioConFallback()) {
        this.router.navigate(['/estimator/client/list']);
      }
    } catch (e: any) {
      this.confirmarInvitacion.set(false);
      this.toast.show(
        this.translate.instant('admin_users.invite_saved_mail_failed', {
