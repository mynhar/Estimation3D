...
-- Marca un contrato como completado (estado → 'completado')
-- y registra actualizado_en.
-- Solo lo puede llamar el rol 'administrador'.
-- ================================================================
CREATE OR REPLACE FUNCTION public.completar_contrato_admin(
  p_contrato_id uuid
)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_estado <> 'en_ejecucion' THEN
    RAISE EXCEPTION 'Solo se puede completar un contrato en estado ''en_ejecucion'' (estado actual: %).', v_estado;
  END IF;

  -- 3. Marcar el contrato como completado
